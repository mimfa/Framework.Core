...
namespace MiMFa.Controls.WinForm.ScrollBar
{
    ...
    [DefaultEvent("Scroll")]
    public partial class HorizontalScrollBar : UserControl
    {
        ...
        {
            get => Slipper.Location.X;//Scroller.GetValue(0, IntMaximum);
        ...
        public int SlipperThickness => Scroller.SlipperLenght < 1 ? 15 : Math.Max(15,Convert.ToInt32(Width * Scroller.SlipperLenght / Math.Max(Scroller.SlipperLenght, Scroller.ScopeLength)));
        ...
        private void Slipper_MouseMove(object sender, MouseEventArgs e)
        {
            if (DraggedTol != 0)
            {
                SlipperLocation += e.X - DraggedTol;
        ...
        {
            float tol = e.Location.X - (Slipper.Location.X + (Slipper.Width / 2F));
            if (tol == 0) return;
            _TrackChange = Convert.ToInt32(tol / 10);
            if (tol > 0)
                if (e.Location.X >= p_Slipper.Width - ((p_Slipper.Width - (Slipper.Location.X + Slipper.Width)) / 2))
                    _TrackLargeChange = true;
                else
                    _TrackLargeChange = false;
            else if (e.Location.X <= Slipper.Location.X / 2)
                _TrackLargeChange = true;
            else
                _TrackLargeChange = false;
        ...
        {
            int max = p_Slipper.Width - Slipper.Width;
        ...
        {
            Slipper.Location = new Point(Scroller.GetValue(0, p_Slipper.Width - Slipper.Width), Slipper.Location.Y);
        ...
        public void UpdateSlipperSize()
        {
            ...
            Slipper.Width = SlipperThickness;
        }

        int LatestHeight = 5;
        private void ScrollBar_Enter(object sender, EventArgs e)
        {
            if (!Hideable || Height == ShowThickness) return;
            LatestHeight = Height;
            Height = ShowThickness;
        }
        private void ScrollBar_Leave(object sender, EventArgs e)
        {
            if (!Hideable || Height == LatestHeight) return;
            Height = LatestHeight;
        }

        private void HorizontalScrollBar_SizeChanged(object sender, EventArgs e)
        {
            UpdateSlipperSize();
        }
    }
